...
class DCA(nn.Module):
    ...
    def m_sum(self, x, y):
        # 对齐张量尺寸，确保它们具有相同的空间维度
        for xi, xj in zip(x, y):
            # 使用上采样或插值将所有张量调整到相同的尺寸
            if xi.shape != xj.shape:
                xj = F.interpolate(xj, size=xi.shape[2:], mode='bilinear', align_corners=False)
            xi = xi + xj  # 此时 xi 和 xj 的尺寸已经一致
        return [xi for xi in x]  # 返回加权后的结果
...
if __name__ == '__main__':
    ...
    input1 = torch.randn(1, features[0], 1024, 1024)
    input2 = torch.randn(1, features[1], 512, 512)
    input3 = torch.randn(1, features[2], 256, 256)
    input4 = torch.randn(1, features[3], 128, 128)
