...
import timm
from PlugNPlay import S2Attention, LocalAttention, CGAFusion, MCAM, HRAMi, CMA_Block
from dca import DCA
from msaa import MSAA

def resize_tensor(input_tensor):
    # 使用F.interpolate来调整大小，指定模式为'nearest'或'bilinear'（对于图像来说）
    return F.interpolate(input_tensor, size=(1024, 1024), mode='bilinear', align_corners=False)
...
class MSA(nn.Module):  # 米字注意力
    ...
    def diag_pool(self, x):
        ...
        for offset in range(-H + 1, W):
            d1 = x.diagonal(offset=offset, dim1=2, dim2=3)  # ↘方向
            diag1.append(d1.mean(-1, keepdim=True))  # [B, C, 1]

        for offset in range(0, H + W - 1):
            flipped = torch.flip(x, dims=[3])  # 水平翻转
            d2 = flipped.diagonal(offset=offset - W + 1, dim1=2, dim2=3)  # ↙方向
            diag2.append(d2.mean(-1, keepdim=True))  # [B, C, 1]

        d1_pool = torch.stack(diag1, dim=2).unsqueeze(-1)  # [B, C, len, 1]
        d2_pool = torch.stack(diag2, dim=2).unsqueeze(-1)

        return d1_pool, d2_pool  # 都是 [B, C, D, 1]

    def forward(self, x):
        loc = self.dwconv_hw(x)
        x_h = self.pool_h(x)  # [B, C, H, 1]
        x_w = self.pool_w(x)  # [B, C, 1, W]
        d1_pool, d2_pool = self.diag_pool(x)  # [B, C, D, 1], [B, C, D, 1]
        d1_pool = d1_pool.squeeze(-1)
        d2_pool = d2_pool.squeeze(-1)

        # 插值回到同样大小（或concat也可）
        h, w = x.shape[2:]
        x_d1 = F.interpolate(d1_pool, size=(h, 1), mode='bilinear', align_corners=False)
        x_d2 = F.interpolate(d2_pool, size=(h, 1), mode='bilinear', align_corners=False)

        x_shi = x_h + x_w  # [B, C, H, 1]
        x_xie = x_d1 + x_d2
        # x_xie = x_xie.squeeze(-1)
        # x_xie = F.interpolate(x_xie, size=(1024, x.shape[-1]), mode='bilinear', align_corners=False)

        ge0 = self.excite(x_shi)  # [B, C, H, 1] → attention map
        ge1 = self.excite(x_xie)
        ge = ge0 + ge1

        return loc * ge  # 注意力作用于输入
class RCA(nn.Module):
    def __init__(self, inp, kernel_size=1, ratio=1, band_kernel_size=11, dw_size=(1, 1), padding=(0, 0), stride=1,
                 square_kernel_size=2, relu=True):
    ...
    def forward(self, x):
        loc = self.dwconv_hw(x)
        att = self.sge(x)
        # print(loc.shape, att.shape)
        # loc = resize_tensor(loc)
        out = att * loc
...
class RCM(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # self.token_mixer = nn.Sequential(token_mixer(dim, band_kernel_size=dw_size, square_kernel_size=square_kernel_size,
        #                                ratio=ratio), eca(dim, k_size=3))
        ...
        self.se = nn.Sequential(
            ...
            nn.Conv2d(dim, max(dim // 16, 1), 1),  # 保证至少是1
            nn.ReLU(inplace=True),
            nn.Conv2d(max(dim // 16, 1), dim, 1),
            nn.Sigmoid()
        )
